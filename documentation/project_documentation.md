...
Streamlit was chosen, instead of alternatives of Dash dashboards or Panel, due to low configuration overhead, fast time to completion and powerful data-driven exploratory workflow capabilities. Dash provides fine-grained frontend control and Panel enables multiple backends but, both come with additional architectural complexity which is not required for the purposes of this project. Streamlit’s declarative programming model enables analytical logic and visualisation to stay tightly coupled to maximize readability, reproducibility, and maintainability and makes it straightforward to deploy to a cloud-hosted environment.
...
The dashboard stays responsive under average interaction patterns regarding performance. Query-level caching and pagination significantly reduce redundant database access, especially when switching tabs or adjusting filters. While very large time windows may introduce minor latency, performance is fine for the exploratory phase of use. Usability is ensured through distinct analytical contexts between tabs, intuitive filtering controls, and consistent interaction patterns. Instead of arranging visualizations through charts, grouping them by data domain minimizes cognitive load while allowing for progressive exploration.

## 4.5 Reflection & Future Improvement

This project demonstrates how publicly available energy and environmental data can be integrated into a coherent interactive model to serve exploratory analysis. A number of key insights concerning data availability, methodological scope, and system extensibility emerged over the development process that inform potential future improvements. The present system, from the perspective of data, has limitations because it is based on a set of regional aggregated datasets. While the regional data alone will be adequate for the analysis at the time it's needed, it provides a mask for localised behaviour of the electricity system. Finer spatial resolution data, for instance the demand per distribution network operator (DNO), substation-level flows or renewable generation at asset level, may be added to future analysis. This could facilitate the regional analysis of disparities and infrastructure constraints, although there is also a potential for additional difficulties related to data volume, governance and privacy.

Ethical issues informed both design of this project and the wide breadth of data analysis. Transparency was a top priority and we clearly documented data provenance, methodological choices and known limitations so that individuals could readily interpret how we inferred instead of receiving outputs as vague indicators. Extra efforts were made to avoid misleading inferences by separating exploratory analysis from causal or predictive claims when dealing with statistical models and correlation metrics. This means there is responsible analytics that balances analytical power and restraint and allows the dashboard to support informed interpretation rather than overconfident conclusions, particularly among a non-technical reader

Expanding the range of data sources would also add value in the analysis. Possible extensions include adding electricity price data, fuel cost indices, grid constraint indicators, and planned outage schedules. Offering context-relevant datasets for energy system restructuring may also be worthwhile, such as renewable subsidy timelines or carbon pricing changes. The present dashboard features descriptiveness and exploration, with experimental analysis of regression and tree-based models. This model can be extended in future versions to add additional time-series statistical models (e.g. ARIMA, state-space models, seasonal decomposition methods) to model temporal structure explicitly. Such approaches would formally enable users to separate the trend, seasonality, and residual variation. The incorporation of probabilistic and uncertainty-aware models would go beyond point estimates. Other techniques, like quantile regression, Bayesian time-series models or prediction interval estimation, would allow users to build their arguments around uncertainty while also not just having deterministic outputs. This would be especially useful with regards to volatile variables like wind generation or carbon intensity

As far as system perspective is concerned, scalability and customisation could be a target for future development. This could involve asynchronous data updates, more sophisticated caching strategies, and also users to save analytical configurations, or export outputs of derived datasets. Explore further the exploratory modelling function by introducing user-defined feature engineering in the experimental tab. Broadly to focus on trade-offs between analytical ambition, data availability, and system complexity, the project is informative. While the current implementation does successfully satisfy its exploratory aims, these future improvements are the next logical step in the direction of more in-depth analysis and a greater potential for application without sacrificing methodological transparency.

## 4.6 Conclusion

Overall, this project demonstrates that it is possible to integrate fragmented UK energy and environmental data into one system, which is interactive, making the system transparent and reproducible in terms of exploratory analyses across regions and time. The dashboard allows viewers to examine system-wide trends in electricity demand, carbon intensity, generation mix, and weather conditions, without the need for specialist technical knowledge - a clear gap between operational monitoring tools and static reporting platforms. Yet significant issues remain unsolved in the case of fine-grained spatial analysis, causal attribution, and robust forecasting under uncertainty. These limitations represent not defects of the design, but rather intentional methodological and data-driven restrictions, thus underpinning a design that recasts the system's role as an exploratory analytical resource, not a predictive or decision-making tool.
